...
class Game:
    ...
    def get_pawn_melee_attacks(self, pawn, j,
                               i):  # copire coller de get_pawn_available_moves mais on veut un pion dans les coins
        available_attacks = []
        (value, form, team) = pawn
        if form == 1 or form == 4:  # c’est un rond
            relative_moves_circle = [(1, 1), (1, -1), (-1, 1), (-1, -1)]
    ...
    def get_melee_attack(self):
        ...
        for y in range(16):
            for x in range(8):
                ...
                for a in attackers:
                    if (math.log(self.board[y][x][0])/math.log(a)).is_integer() or (math.log(a) / math.log(self.board[y][x][0])).is_integer():
    ...
    def get_game_available_attack(self):
        available_attacks = []
        available_attacks += self.get_melee_attack()
        return available_attacks
